<template>
  <div>
    ...
    <base-page-heading
      ...
      subtitle="Only completed candidates can apply in assignments"
    >
      <template #extra>
        <b-breadcrumb class="breadcrumb-alt">
          ...
          >
          <b-breadcrumb-item active>Completed</b-breadcrumb-item>
        </b-breadcrumb>
      </template>
    </base-page-heading>
    <!-- END Hero -->
    ...
    <div class="content">
      <!-- Full Table -->
      <base-block rounded title="Shortlisted Applications">
        <template #options>
            ...
            >
              <b-td class="text-center">
                <a v-b-modal.modal-block-extra-large @click="url = user.resume">
                  <i class="fa fa-file-pdf text-success mr-1 align-middle" color="danger" style="font-size: 1.5em;"></i>
                  </a>
              </b-td>
...
export default {
  data() {
    return {
      loading: [],
      rows: this.$store.state.firestoreData.candidates.shortlisted.length,
      perPage: 5,
      currentPage: 1,
  ...
  computed:{
    filteredArray (){
      return this.$store.state.firestoreData.candidates.shortlisted.slice(((this.currentPage-1)*this.perPage),((this.currentPage)*this.perPage));
    }
  },
  methods: {
    ...
    async deleteEntry(name, email, index){
      ...
      // eslint-disable-next-line no-unused-vars
      await entry.update({
        "candidates.shortlisted": firebase.firestore.FieldValue.arrayRemove(details),
      });
      await entry.update({
