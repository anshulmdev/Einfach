<template>
  <div>
    ...
    <base-page-heading
      ...
      subtitle="All Processed applications"
    >
      <template #extra>
    ...
    <div class="content">
      <!-- Full Table -->
      <base-block rounded title="Completed Applications">
        <template #options>
            ...
            >
              <b-td class="text-center">
                <a v-b-modal.modal-block-extra-large @click="url = user.resume">
                  <i class="fa fa-file-pdf text-primary mr-1 align-middle" color="danger" style="font-size: 1.5em;"></i>
                  </a>
              </b-td>
...
export default {
  data() {
    return {
      loading: [],
      rows: this.$store.state.firestoreData.candidates.completed.length,
      perPage: 5,
      currentPage: 1,
  ...
  computed:{
    filteredArray (){
      return this.$store.state.firestoreData.candidates.completed.slice(((this.currentPage-1)*this.perPage),((this.currentPage)*this.perPage));
    }
  },
  methods: {
    ...
    async deleteEntry(name, email, index){
      ...
      const addInvite = await entry.update({
        "candidates.completed": firebase.firestore.FieldValue.arrayRemove(details),
      });
      this.loading = []
