<template>
  <div>
    ...
    <div class="content">
      <!-- Full Table -->
      <base-block rounded title="Invited Applications">
        <template #options>
            ...
            >
              <b-td class="text-center">
                <a v-b-modal.modal-block-extra-large @click="url = user.resume">
                  <i class="fa fa-file-pdf text-warning mr-1 align-middle" color="danger" style="font-size: 1.5em;"></i>
                  </a>
              </b-td>
...
export default {
  data() {
    return {
      loading: [],
      rows: this.$store.state.firestoreData.candidates.invited.length,
      perPage: 5,
      currentPage: 1,
  ...
  computed:{
    filteredArray (){
      return this.$store.state.firestoreData.candidates.invited.slice(((this.currentPage-1)*this.perPage),((this.currentPage)*this.perPage));
    }
  },
  methods: {
    ...
    async invite(name, email, index) {
      ...
      const invited = {user:name, body: emailTemplate, received: new Date(), title: 'Candidate Shortlist Mail', email}
      this.loading.push(index)
      await fetch(
