    ...
    >
      <template #extra>
        <b-breadcrumb class="breadcrumb-alt">
          ...
          >
          <b-breadcrumb-item active>Rejected</b-breadcrumb-item>
        </b-breadcrumb>
      </template>
    </base-page-heading>
    <!-- END Hero -->
              ...
              <b-td class="text-center">
                <b-btn-group>
                  ...
                  v-if="loading.includes(index)"
                    v-b-tooltip.hover.nofade.left="'Send rejection email'"
...
export default {
  data() {
    return {
      loading: [],
      rows: this.$store.state.firestoreData.candidates.rejected.length,
      perPage: 5,
      currentPage: 1,
  ...
  computed:{
    filteredArray (){
      return this.$store.state.firestoreData.candidates.rejected.slice(((this.currentPage-1)*this.perPage),((this.currentPage)*this.perPage));
    }
  },
  methods: {
    ...
    async deleteEntry(name, email, index){
      ...
      let details = this.$store.state.firestoreData.candidates.rejected[index];
