    ...
    <b-row>
      <b-col md="5" xl="3">
        ...
        <div class="d-md-none push">
          ...
          <b-button variant="primary" block v-toggle-class="{ targetId: 'one-inbox-side-nav', class: 'd-none' }"> Inbox Menu </b-button>
        </div>
        ...
        <div id="one-inbox-side-nav" class="d-none d-md-block push">
          ...
          <base-block title="Inbox" header-class="block-header-default" rounded>
            <template #options>
              <b-button variant="alt-primary" size="sm" class="btn btn-sm btn-alt-primary" v-b-modal.one-inbox-new-message> <i class="fa fa-pencil-alt mr-1"></i> Compose </b-button>
            </template>
            <b-nav class="font-size-sm push" pills vertical>
              ...
              >
                ...
                <b-badge pill variant="secondary">{{ totalMails }}</b-badge>
              </b-nav-item>
              <b-nav-item
                @click="
                  label = 'invited'
              ...
              >
                <span> <i class="fa fa-fw fa-paper-plane mr-1"></i> Invited </span>
                <b-badge pill variant="secondary">{{ $store.state.firestoreData.inbox.invited ? $store.state.firestoreData.inbox.invited.length : 0 }}</b-badge>
              </b-nav-item>
              <b-nav-item
                @click="
                  label = 'shortlisted'
              ...
              >
                <span> <i class="fa fa-fw fa-pencil-alt mr-1"></i> Custom </span>
                <b-badge pill variant="secondary">{{ $store.state.firestoreData.inbox.custom ? $store.state.firestoreData.inbox.custom.length : 0 }}</b-badge>
              </b-nav-item>
            </b-nav>
          </base-block>
          <!-- END Inbox Menu -->
      ...
      <b-col md="7" xl="9">
        ...
        <b-modal id="one-inbox-message" size="xl" body-class="p-0" hide-footer hide-header>
          <div class="block block-rounded block-themed block-transparent mb-0">
            ...
            <div class="block-content block-content-full font-size-sm d-flex justify-content-between align-items-center bg-body-light">
              <div>
                <a class="font-w600" href="javascript:void(0)"> {{ inbox[showMessage].email }}</a
                ><br />
            ...
            <div class="block-content font-size-sm">
              <p v-html="inbox[showMessage].body" />
            </div>
          </div>
        </b-modal>
        ...
        <base-block rounded btn-option-fullscreen header-class="block-header-default">
          <template #title> Page {{ currentPage }} out of {{ parseInt(currentMails.length / perPage) + 1 }} </template>
          <template #options>
